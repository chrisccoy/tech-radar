...
'$URL' as "url",
CASE
   WHEN EXISTS (select 1 as foo where ((select count(*) from csv."$CSVTABLE" where "$NAME" like '%NEXT%') + (select count(*) from csv."$CSVTABLE" where "$NAME" like '%Both%')) > (select count(*) from csv."$CSVTABLE" where "$NAME" like '%PAST%') ) THEN 1
